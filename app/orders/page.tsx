...
export default function OrdersPage() {
  ...
  if (error) return <div className="container mx-auto px-4 py-8">Failed to load orders. Please login.</div>;

  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-2xl font-bold mb-6">My Orders</h1>
      {success && (
        <div className="bg-green-100 text-green-700 p-3 rounded mb-4">Order placed successfully! We'll notify you when it ships.</div>
      )}
      {orders?.length === 0 ? (
            ...
            <div key={order._id} className="bg-white rounded-lg shadow-card p-4">
              ...
              <div className="pt-3">
                {order.orderItems.slice(0, 2).map((item: any) => (
                  <div key={item.product._id} className="flex gap-3 text-sm py-1">
